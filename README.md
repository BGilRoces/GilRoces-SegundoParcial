...
## Error 1 — Bloque `init { return Episode; }` suelto en el modelo de dominio
...
## Error 2 — Nombre del método del Repository en `snake_case` y desfasado con la implementación
...
## Error 3 — Retrofit construido sin `baseUrl()`
...
## Error 4 — Efecto secundario (llamada al ViewModel) dentro del cuerpo del Composable
...
## Error 5 — La Activity usa `MaterialTheme` plano en vez del tema propio `SimpsonsAppTheme`
...
## Error 6 — Código de plantilla muerto que rompe la arquitectura y los tests

- **Dónde:** [data/DataRepository.kt](app/src/main/java/com/example/simpsonsapp/data/DataRepository.kt) (`DataRepository` / `DefaultDataRepository` que emite `listOf("Android")`) y [main/MainScreenViewModel.kt](app/src/main/java/com/example/simpsonsapp/main/MainScreenViewModel.kt) (`MainScreenViewModel` + `MainScreenUiState`). Ninguno se usa en la app real (que usa `MainViewModel`), pero el test [test/.../MainScreenViewModelTest.kt](app/src/test/java/com/example/simpsonsapp/ui/main/MainScreenViewModelTest.kt) sí depende de ellos.
- **Por qué está mal:** son restos del esqueleto/plantilla del proyecto. Conviven dos "MainViewModel" (`MainViewModel` real y `MainScreenViewModel` fantasma), lo que viola la **separación de responsabilidades** y la **Single Source of Truth**, confunde la lectura y deja **código muerto**. Además `MainScreenViewModel` no es `@HiltViewModel` ni se inyecta, rompiendo la coherencia MVVM del resto del proyecto.
- **Solución propuesta:** eliminar `DataRepository`, `DefaultDataRepository`, `MainScreenViewModel`, `MainScreenUiState` y los tests que los referencian (o reescribir los tests apuntando a `MainViewModel`). Dejar un único ViewModel por pantalla.

---

## Error 7 — Lógica de negocio y "números mágicos" dentro de la UI (selector de episodios)
...
## Error 8 — `EpisodeCard` clickeable con `onClick = { }` vacío en la pantalla de detalle
...
## Error 9 — Recolección de `StateFlow` no consciente del ciclo de vida (`collectAsState`)
...
## Error 10 — Test instrumentado desincronizado con la firma real de `MainScreen`

- **Dónde:** [androidTest/.../MainScreenTest.kt:18-24](app/src/androidTest/java/com/example/simpsonsapp/ui/main/MainScreenTest.kt#L18-L24) hace `MainScreen(FAKE_DATA)` con `FAKE_DATA: List<String>` y verifica textos `"Hello $it!"`.
- **Por qué está mal:** la firma real es `MainScreen(onNavigateToDetail: (Int) -> Unit, viewModel: MainViewModel)` (ver [main/MainScreen.kt:40-43](app/src/main/java/com/example/simpsonsapp/main/MainScreen.kt#L40-L43)), por lo que pasar un `List<String>` es un **error de compilación** del test. Además los textos `"Hello ..."` no existen en la pantalla, así que la aserción nunca podría pasar. Es otro resto de plantilla que invalida la suite de tests.
- **Solución propuesta:** reescribir el test acorde a la firma real: pasar un lambda `onNavigateToDetail = {}` y un `MainViewModel` de prueba (o un fake del repositorio), y asertar contra textos que la pantalla realmente renderiza (por ejemplo el título `"The Simpsons Episodes"`).
